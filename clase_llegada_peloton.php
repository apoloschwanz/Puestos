<?php
	class llegada_peloton extends entidadj {
		...
		{	
			...
			$this->lista_campos_lista[]=new campo_entidad( 'Orden' 	, 'number' 	, 'Orden'  ) ;
			//
			//
			...
			$this->lista_campos_lectura[]=new campo_entidad( 'Orden' 	, 'number' 	, 'Orden'  ) ;
			$this->lista_campos_lectura[3]->pone_readonly();
			//
			// Nombre de la tabla
	...
	protected function mostrar_alta_peloton()
		{
			$botones = '<input type="submit" name="'.$this->prefijo_campo.'OkGrabarPeloton" value="Agregar Peloton" >';
			$pagina=new paginaj($this->pagina_titulo,$botones);
			$pagina->pone_valor_oculto( $this->okListaPosicion , $this->desde );
			$txt = 	$this->texto_agregar_peloton();
			$pagina->insertarCuerpo($txt);
			$pagina->sinborde();
			$pagina->graficar_c_form($_SERVER['PHP_SELF']);
				
		}
	public function texto_agregar_peloton()
		{
			$this->error= false ;
			$cpo = new Campo();
			$this->Leer();
			...
			else
			{
				...
				// Abre tabla
				$txt = '<table>';
				$tn_CntCamp = 10 ; // Cantidad de Registros a agregar juntos
				
				
				for($j=0;$j<$tn_CntCamp;$j++)
				{
					$txt.='<tr>';
					//$txt.='<td>';
					if( $j == 0 ) $cpo->pone_autofocus();
					else $cpo->saca_autofocus();
					$cpo->pone_nombre( $this->prefijo_campo.'id_CorrPel_'.$j.'_' ) ;
					$cpo->pone_valor( '' ) ;
					$cpo->pone_tipo( 'number' ) ;
					$txt.=$cpo->txtMostrarParaModificar() ;
					//$txt.='</td>';
				...
				// Tiempo Peloton
				$txt.='<tr>';
				$cpo->pone_nombre( $this->prefijo_campo.'tiempo_Cor_Pel' ) ;
					$cpo->pone_valor( $this->registro['Tiempo'] ) ;
					$cpo->pone_tipo( 'datetime' ) ;
					//$txt.=$cpo->txtMostrarParaModificar() ;
	...
	protected function Carga_Sql_Lista()
	{
		...
		{	
			...
			//
			// Post Carga Peloton
			$OkGrabarPeloton = $this->prefijo_campo.'OkGrabarPeloton' ;
			$OkTiempoCorPel = $this->prefijo_campo.'tiempo_Cor_Pel' ;
			//
			// Post Generales
			...
			elseif ( isset( $_POST[$this->okGrabaAgregar] ) )
			{
				...
				else $this->mostrar_alta_peloton();
			}
			elseif ( isset($_REQUEST['okSalir'] ) )
			{
				$this->ok_Salir() ;
			}
			elseif ( isset($_REQUEST['ok'] ) ) $tb_armar_pagina = true ;
			...
			elseif ( isset($_POST[$OkGrabarPeloton]) or isset($_POST[$OkTiempoCorPel]) )
			{
				$this->texto_agregar_peloton_okGrabar();
				if ( $this->hay_error() == true ) $this->muestra_error() ;
				else $tb_armar_pagina = true ;
			}
			else
			{
				$tb_armar_pagina = true ;
		...
		public function texto_agregar_peloton_okGrabar() 
		{
			$primerCampo = true;
			$lst_val = '';
			$OkTiempoCorPel = $this->prefijo_campo.'tiempo_Cor_Pel' ;
			$ValorTiempo = $_POST[$OkTiempoCorPel];
			$hayvalor = false;
			//var_dump($_POST);
			//echo '<br>';
			//var_dump($OkTiempoCorPel);
			$orden = 0;
			for($i=0;$i<30;$i++)
			{
				$OkIdCorrPel=$this->prefijo_campo.'id_CorrPel_'.$i.'_' ;
				//echo '<br>';
				//var_dump($OkIdCorrPel);
				if ( isset($_POST[$OkIdCorrPel])) $IdCorrPel = $_POST[$OkIdCorrPel];
				else $IdCorrPel = '' ;
				if ( ! empty($IdCorrPel) )
				{
					$orden++;
					if ( $primerCampo == false ) $lst_val = $lst_val.', ' ;
					else $primerCampo = false ;
					$lst_val.= "(".$IdCorrPel.", '".$ValorTiempo."', ".$orden.")" ;
					$hayvalor = true;
				}
			}
			if ( $hayvalor )
			{
				$this->strsql = ' INSERT INTO '.$this->nombre_fisico_tabla.' ( Corredor_Id,Tiempo,Orden) VALUES ';
				$this->strsql .= $lst_val ;
				$this->ejecuta_sql();
			}
			/*
